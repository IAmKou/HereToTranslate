...
export default {
  ...
  methods: {
    fetchRequests() {
      axios.get((import.meta.env.VITE_API_URL || 'http://localhost:3000/api') + '/requests').then((res) => {
        this.requests = res.data;
      });
    },
    handleReview(id, status) {
      axios
        .patch((import.meta.env.VITE_API_URL || 'http://localhost:3000/api') + '/requests/review', { id, status })
    ...
    handleEdit(request) {
      ...
      if (updated !== null) {
        axios
          .patch((import.meta.env.VITE_API_URL || 'http://localhost:3000/api') + '/requests', {
            id: request.id,
            description: updated,
          })
          .then(() => this.fetchRequests());
    ...
    handleView(id) {
      axios
        .get((import.meta.env.VITE_API_URL || 'http://localhost:3000/api') + `/requests/${id}`)
        .then((res) => alert(JSON.stringify(res.data, null, 2)));
    },
    handleCancel(id) {
      if (confirm('Are you sure you want to cancel this request?')) {
        axios
          .delete((import.meta.env.VITE_API_URL || 'http://localhost:3000/api') + `/requests/${id}`)
          .then(() => this.fetchRequests());
      }
    },
    setSort(key) {
...
/* If used within a layout with collapsed sidebar */
