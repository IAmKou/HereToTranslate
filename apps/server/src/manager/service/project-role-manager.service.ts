...
export class ProjectRoleManagerService extends CommonHttpServiceImpl {
  ...
  async createProjectRole(
    ...
    this.logger.debug(
      'Check PermissionFlags.ManageRoles:',
    ...
    const hasProjectAdmin =
      (BigInt(permissionFlags) & PermissionFlags.ProjectAdmin) ===
  ...
  async fixEveryoneRolePermissions(projectId: bigint) {
    ...
    try {
      ...
      this.logger.debug(
        `Everyone role permissions fixed for project [${projectId}]`
      );
    } catch (error) {
      this.unknownErrorHanlder(
        error,
        'Failed to fix Everyone role permissions'
      );
    }
  }
}
