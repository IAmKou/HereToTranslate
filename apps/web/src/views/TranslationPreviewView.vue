...
const loadAvailableLanguages = async (fileId: string) => {
  try {
    // Get available languages for this file from backend
    const response = await axiosInstance.get(`/translation/languages/${fileId}`);
    const languages = response.data.languages || [];

    // Convert language codes to display names
    availableLanguages.value = languages.map((lang: string) => ({
      code: lang,
      name: languageNames[lang] || lang
    }));

    console.log('Available languages for file:', availableLanguages.value);
  } catch (err: any) {
...
const loadFileInfo = async () => {
  ...
  try {
    ...
    await loadAvailableLanguages(fileId);
    ...
    // Set language from query params or default to first available language
    if (route.query.language) {
      ...
      selectedLanguage.value = 'en'; // Fallback to English
    }
...
const loadPreview = async () => {
  ...
  try {
    ...
    // Backend returns preview as string (UTF-8 for text files, base64 for binary files)
    ...
    if (fileType === 'application/pdf') {
      ...
      previewType.value = 'text';
      // For text files (text/plain, application/json, etc.), preview is UTF-8 string
      previewData.value = preview;
    }
...
const renderDocxWithPreview = async () => {
  ...
  try {
    ...
    if (docxContainer.value) {
      docxContainer.value.innerHTML = `<div style="color: red; padding: 20px;">Error rendering DOCX file: ${error}</div>`;
    }
  }
};
