...
import { CommitEntity } from '#LocalProject/Entities';
...
export class FileService {
  ...
  async saveFile(params: {
    ...
    this.logger.log(
      `Saving file: ${fileName}, type: ${fileType}, projectId: ${projectId}, branchId: ${branchId}, uploader: ${uid}`
    );
    ...
    this.logger.log(
      `fileName (Buffer): ${Buffer.from(fileName, 'utf8').toString('hex')}`
    );
  ...
  async deleteFile(fileId: bigint, userId: string | bigint) {
    ...
    if (!hasAttachFiles) {
      throw new ForbiddenException(
        'You do not have permission (AttachFiles) to delete this file'
      );
    }
    ...
    try {
      ...
      if (file.project && file.branch) {
        ...
        try {
          ...
          appendLog(
            'Error pushing manifest to GitHub: ' + (err?.message || err)
          );
        }
      }
      ...
      await this.fileRepository.save(file);
      this.logger.error(
        `Error generating manifest for file ${file.fileName}:`,
        error
      );
  ...
  async handleLocalUpload(
    ...
    if (fileEntity) {
      try {
        ...
        this.logger.error(
          `Failed to generate manifest for file ${fileEntity.id}:`,
          error
        );
      ...
      if (fileEntity.project && fileEntity.branch) {
        ...
        try {
          ...
          this.logger.log(
            `Manifest pushed to repo for fileId: ${fileEntity.id}`
          );
        } catch (err) {
          this.logger.error('Error pushing manifest to GitHub', err);
        }
